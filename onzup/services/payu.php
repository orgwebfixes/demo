 ...
 * @throws Exception
 */
function pay_online($params, $salt) {
    if (count($_POST) && isset($_POST['mihpayid']) && !empty($_POST['mihpayid'])) {
        $_POST['surl'] = $params['surl'];
        $_POST['furl'] = $params['furl'];
        $result = payu_response($_POST, $salt);
        return Misc::show_reponse($result);
    } else {
        $host = (isset($_SERVER['https']) ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];

        if (empty($params['surl']) && isset($_SERVER['REQUEST_URI']) && !empty($_SERVER['REQUEST_URI'])) {
            $params['surl'] = $host;
        }
        if (empty($params['furl']) && isset($_SERVER['REQUEST_URI']) && !empty($_SERVER['REQUEST_URI'])) {
            $params['furl'] = $host;
        }
 ...
 * @return number
 */
function payu_response($params, $salt) {
    if (!is_array($params)) {
...
function verify_payment($Transaction_ID) {
// Merchant key here as provided by Payu
    $key = "Wt0IvL";
    $salt = "joCT7wmH";
    $command = "verify_payment";
    $var1 = $Transaction_ID; // Transaction ID
